...
Machinelearning problems are classified into supervised and unsupervised based on if the training data is labeld or not. Supervised problems are then split up again based on if the values are descrete or continupus. These are called classificarion and regression.
The support vector machine is aimed at solving the classification problem. It is first trained with a datasset of known labels, this trained model can then be used to assign labels to new data.

In machinelearning \red{we speek} about classification 
