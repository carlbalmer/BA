...
\item \textbf{Fingerprinting data} only labelled with the room number
 ...
 A set of 44 \textbf{evenly distributed} samples labelled with \((X,Y)\)-coordinates.
\end{itemize}
...
Cross validation can sometimes be a little biased. So the impact of magnetic field data was also compared by training with the \emph{Grid} data set and testing against the \emph{XY} data set, mincing a real-world use case of the room recognition. Surprisingly this only resulted in a 2.3\% improvement.
